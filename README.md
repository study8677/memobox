...
3. 如果任务明确需要跨项目经验、项目 index 没有可用记录，或者属于可复用的安装、认证、CI、部署、插件、工具链或事故处理任务，再扫描全局 index；不要因为全局库存在就读取所有正文。
4. 只有需要证据时才读取 `traces/<id>.jsonl`。区分“打开过的 id”和“实际复用的 id”：只有真正影响计划、决策、实现或验证的记忆才算复用。
5. 任务结束执行价值门槛。只保存稳定决策、非显而易见且已验证的根因/修复、可复用的精确证据、重要风险或跨会话交接；大多数任务应写零条或一条。
6. 新记忆如果复用了项目记忆，用 `--source-ref "memobox:<id>"`；复用了全局记忆则用 `--source-ref "memobox-global:<id>"`。不要为了记录复用本身而额外创建低价值记忆。
...
真实任务完成后，用严格记录器把 opened/reused ids 和实测指标追加到被忽略的 `work/`，再用同一个 task manifest 汇总；记录器不会生成任务或猜测结果：
...
| `memobox write --supersedes <id>` | 写入替代记录、回链来源，并把被替代记录标记为 `stale` |
...
全局 index 不是第二个默认上下文包。只在任务需要可迁移的跨项目经验时扫描它，并分别记录项目/全局记忆的 opened ids 与 reused ids。全局记录应保持少而精，禁止晋升一次性项目状态、私有路径、推测、秘密、个人数据或机密 raw trace。
...
`last_verified_at` 和 `valid_until` 会暴露在轻量 index 中，供模型判断新鲜度；超过 `valid_until` 不会后台自动改变状态。`--supersedes` 会验证来源、创建回链，并把被替代记录标记为 `stale`。
...
晋升前必须打开正文确认：结论已验证、离开当前 workspace 仍然成立、无需私有仓库背景也能理解，并且不含敏感数据。`promote` 会复制出新的全局 id，并用项目 store 的绝对路径保留可追溯来源；它不会自动同步两边后续变化。
...
- 跨项目任务：按需扫描全局 index，项目/全局 opened ids 和 reused ids 分开记录
- 任务结束：通过价值门槛后才用 `memobox write` 写结构化结果；实际复用的 id 写入 `source_refs`
- 结论替代：用 `--supersedes` 回链并 stale-mark 旧记录；用 verification/validity 字段暴露新鲜度
...
- [ ] 更好的 index 分组和生命周期视图
- [x] 显式 supersession、验证时间和有效期信号
- [ ] 自动 stale memory detection
...
- [x] opened/reused 分离的真实 dogfood run 记录器
